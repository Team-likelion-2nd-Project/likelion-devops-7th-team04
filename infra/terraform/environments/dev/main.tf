...
# 예전엔 이 값이 main.tf에 문자열로 직접 박혀 있었다(www.team04hotel.kro.kr 고정).
# dev/prod가 서로 다른 서브도메인을 쓰게 되며, main.tf 자체는 dev/prod가 동일한 코드를
# 유지하도록 값을 여기서 선언만 하고 terraform.tfvars(환경별 로컬 파일, git 미추적)에서
# 주입받는다 — 기본값을 두지 않아 tfvars가 없으면 apply가 실패한다(다른 필수 변수들과
# 동일한 패턴). 실제 도메인이 붙으려면 kro.kr DNS에 CNAME이 별도로 등록되어 있어야
# 한다(Terraform 밖, 리포에 aws_route53_record가 없어 수동 관리).
...
module "s3_frontend" {
  ...
  # DEV-171: CloudFront에 alias로 연결. DNS(kro.kr)는 팀원이 별도로 CNAME 등록 — 여기서는
  # CloudFront/ACM 쪽만 처리. 실제 도메인 값은 var.domain_name/var.certificate_domain으로
  # terraform.tfvars에서 주입된다(위 변수 선언부 주석 참고).
...
module "eks" {
  ...
  # label selector 대상 네임스페이스 — compute=fargate 라벨이 있는 pod(llm-service)만
  # 여기서 Fargate로 뜨고, ollama는 GPU node group, n8n은 DEV-196에서 api_cpu node
  # group으로 감(위 CPU Fargate Profile 주석 참고)
  ...
  # -------------------------
  # DEV-196: api-gateway/n8n 전용 EC2 CPU Node Group
  # -------------------------
  ...
  # (infra/terraform/environments/dev/addons/cluster-autoscaler.tf)가 이 노드그룹만
