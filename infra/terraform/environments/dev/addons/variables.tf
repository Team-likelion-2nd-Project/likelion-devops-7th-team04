...
# DEV-170: Pod Identity 대신 IRSA로 전환하며 Helm release가 ServiceAccount에
# eks.amazonaws.com/role-arn annotation을 직접 달아줘야 한다. 이 addons root는
# ../main.tf(메인 dev root)와 별도 state라 module.iam을 직접 참조할 수 없어
# terraform.tfvars에 값을 수동으로 채운다(기존 vpc_id와 동일한 패턴).
